...
Requires `.accessibility/<target>/04-remediate.md` from the Remediate phase. Use `--skip-checks` to bypass.
...
6. Writes results to `.accessibility/<target>/05-retest.md`

## Output

Creates `.accessibility/<target>/05-retest.md` containing:
